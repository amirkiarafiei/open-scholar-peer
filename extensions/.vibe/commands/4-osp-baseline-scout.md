...
4. The skill independently searches for state-of-the-art methods on the same task and benchmarks. Tools: `.open-scholar-peer/osp` CLI (or `.open-scholar-peer\osp.cmd` on Windows), native Web Search. Targeted queries include leaderboards, benchmark suites, and recent SOTA claims.
