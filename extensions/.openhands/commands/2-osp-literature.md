...
⚠️ Each invocation makes ~8-12 API calls across 3 databases (arXiv, Semantic Scholar, Google Scholar), called **one at a time**. Wall-clock time per round depends on provider latency — a slow or rate-limited provider can add up to `OSP_CALL_TIMEOUT` seconds (default 180s). To cap wait time, set `OSP_CALL_TIMEOUT=30` in your `.env`.
...
3. Run the **next pending round only**:
   ...
   - The skill searches each provider **separately, in order**: `search-arxiv` → `search-semantic-scholar` → `search-google-scholar`, then native web search. **Never use `search-all`** — individual calls allow partial results and early progress reporting if one provider hangs.
