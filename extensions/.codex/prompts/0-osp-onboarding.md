...
writes: [".brain/session/raw/00_review_guidelines.md", ".brain/session/raw/05_qa_<slug>.md (per criterion)", ".brain/session/session.json"]
---
...
- Read `.brain/session/session.json`. If missing, run `scripts/init_brain.sh` first or initialize a default per the v2 schema.
...
- Check `.brain/session/input/` for a paper file. Common extensions: `.pdf`, `.md`, `.tex`, `.docx`.
- If empty, ask the user where the paper is. Help them collaboratively — accept any path, then copy the file into `.brain/session/input/`.
- **Always produce `.brain/session/input/paper.md`** (the canonical readable form):
  - If the original is `.md`, ensure it's named `paper.md` (rename if necessary).
  - If the original is `.pdf` / `.docx` / `.tex`, attempt conversion using the fallback chain (mirror of the CLI Tooling fallback in `osp-rules.md`):
    1. `.brain/runtime/osp convert-pdf <path> .brain/session/input/paper.md` (shim handles venv/uv automatically)
    2. `uv run --script .brain/runtime/convert_pdf.py <path> .brain/session/input/paper.md` (if shim fails and `uv` is available)
    3. `.brain/runtime/venv/bin/python .brain/runtime/convert_pdf.py <path> .brain/session/input/paper.md` (if `uv` unavailable, use installer venv)
    - On Windows replace `/` with `\` in paths and use `osp.cmd` instead of `osp`.

  - If both methods fail, tell the user explicitly that downstream phases require `.brain/session/input/paper.md` and offer to let them provide a manual markdown conversion. Pause until it is in place.
- Save `paper.path` (original) and `paper.parsed_path` (the canonical `.brain/session/input/paper.md`) into `session.json`.
...
3. **Generic fallback:** copy `extensions/_shared/defaults/generic_review_guidelines.md` (or its synced equivalent in your tool's `defaults/` directory) into `.brain/session/raw/00_review_guidelines.md`.
...
Write the retrieved/provided/generic guidelines to `.brain/session/raw/00_review_guidelines.md` using the universal artifact structure (Method / Output / Provenance):
...
For each criterion in `qa_criteria[]`, create `.brain/session/raw/05_qa_<slug>.md` from the template at `defaults/qa_pair_template.md` (or the synced equivalent). Pre-fill:
...
↳ .brain/session/raw/05_qa_<slug>.md  (pre-scaffolded for each criterion)
↳ .brain/session/session.json  (qa_pairs_per_criterion: <N>)
Next: /1-osp-summary
─────────────────────────────────────────────────────────
```
