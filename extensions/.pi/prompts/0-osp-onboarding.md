...
3. **Generic fallback:** copy `.pi/defaults/generic_review_guidelines.md` into `.brain/raw/00_review_guidelines.md`.
...
For each criterion in `qa_criteria[]`, create `.brain/raw/05_qa_<slug>.md` from the template at `.pi/defaults/qa_pair_template.md` (or the synced equivalent). Pre-fill:
