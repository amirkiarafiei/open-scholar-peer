...
- `qa_criteria[]` in `session.json`. If it is empty, onboarding never ran — use the generic criteria from
  `.pi/defaults/generic_review_guidelines.md` and record that you did.
...
3. For each criterion in `qa_criteria[]`:
   - Open or initialize `.brain/raw/05_qa_<criterion_slug>.md` from `.pi/defaults/qa_pair_template.md`.
