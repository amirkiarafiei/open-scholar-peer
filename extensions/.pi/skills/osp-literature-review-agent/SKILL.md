...
Print the **opening block** defined in `.pi/defaults/phase_block_template.md`, labelled
...
failed. The template at `.pi/defaults/round_strategy_template.md` has a line for each.
...
Use `.pi/defaults/round_strategy_template.md` as the skeleton for each round. Fill in:
