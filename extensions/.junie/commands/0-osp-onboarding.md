...
- **Always produce `.brain/session/input/paper.md`** (the canonical readable form):
  ...
  - If the original is `.pdf` / `.docx` / `.tex`, attempt conversion using `uvx markitdown <path> > .brain/session/input/paper.md` or by running our helper script `python3 .brain/runtime/convert_pdf.py <path> .brain/session/input/paper.md` (or `python3 .brain/runtime\convert_pdf.py <path> .brain/session/input/paper.md` on Windows).
