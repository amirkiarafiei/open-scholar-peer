...
1. **Prefer individual provider calls over `search-all`.** We strongly recommend calling each provider subcommand separately, in sequence (`search-arxiv` → `search-semantic-scholar` → `search-google-scholar`), rather than using `search-all`.
   Reason: providers have different latency profiles. If one provider (e.g. Semantic Scholar on the anonymous tier timing out at 180 s) hangs, `search-all` will block the entire search. Sequential separate calls allow the agent to capture partial results and report progress immediately.
