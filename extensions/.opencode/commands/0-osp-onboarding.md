...
3. **Generic fallback:** copy `.opencode/defaults/generic_review_guidelines.md` into `.brain/raw/00_review_guidelines.md`.
...
For each criterion in `qa_criteria[]`, create `.brain/raw/05_qa_<slug>.md` from the template at `.opencode/defaults/qa_pair_template.md` (or the synced equivalent). Pre-fill:
