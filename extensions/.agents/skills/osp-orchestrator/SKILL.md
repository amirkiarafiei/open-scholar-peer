...
description: >
  ...
  references any /N-osp-* command. Owns the brain protocol: read .brain/session/session.json
...
1. **Read `.brain/session/session.json` first.** If missing, the user hasn't run `/0-osp-onboarding` yet — tell them to run it.
...
| (all complete) | — | Print location of `.brain/session/review/final_review.md` and ask if user wants to revise any phase |
...
- **No paper loaded.** User invokes `/1-osp-summary` but `.brain/session/input/` is empty. Help them locate the file collaboratively (this is an agentic environment — be proactive).
