...
Render the **opening block** exactly as `.agents/defaults/phase_block_template.md` defines it — that file
...
3. Run the **next pending round only**:
   ...
   - The skill lists the retrieval tools this project installed, picks the ones that suit the paper's
     ...
     query formulations**.
   - Write the round file (`02a`, `02b`, or `02c`) using the template at `.agents/defaults/round_strategy_template.md`.
...
5. Print the **closing block** from `.agents/defaults/phase_block_template.md`, labelled
