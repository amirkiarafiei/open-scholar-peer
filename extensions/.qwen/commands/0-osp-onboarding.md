...
Render the **opening block** exactly as `.qwen/defaults/phase_block_template.md` defines it — that
...
3. **Generic fallback:** copy `.qwen/defaults/generic_review_guidelines.md` into `.brain/raw/00_review_guidelines.md`.
...
For each criterion in `qa_criteria[]`, create `.brain/raw/05_qa_<slug>.md` from the template at `.qwen/defaults/qa_pair_template.md` (or the synced equivalent). Pre-fill:
...
Render the **closing block** from `.qwen/defaults/phase_block_template.md`. **Build the rail from
