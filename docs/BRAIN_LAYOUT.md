...
The `.brain/` directory at the project root is the state store and runtime for OSP. It is gitignored. It is split into two main sections:
- `runtime/`: Stable, persistent runtime (venv, shims, scripts).
- `session/`: Ephemeral review session state (per-paper).

## Tree

```
.brain/
├── runtime/                           Stable, persistent runtime
│   ├── osp                            Unix execution shim
│   ├── osp.cmd                        Windows execution shim
│   ├── osp_cli.py                     Main Python search CLI
│   ├── convert_pdf.py                 Local PDF conversion helper
│   ├── providers/                     API query modules
│   └── venv/                          Python virtual environment
└── session/                           Ephemeral review session state (per-paper)
    ├── session.json                   Single source of truth for run state
    ├── input/                         User drops paper here (any format)
    │   ├── paper.pdf                  (or .docx, .tex, etc.)
    │   └── paper.md                   (parsed via convert_pdf.py, optional)
    ...
    │   ├── 05_qa_<criterion_slug>.md  One file per active criterion (N Q&A pairs)
...
- Criterion slugs use kebab-case derived from the venue's review form (e.g. `novelty`, `technical-soundness`, `clarity`, `significance`, `reproducibility`). The slug list is authoritative in `session/session.json.qa_criteria[].slug`.
- One artifact per file. Never append unrelated content to an existing artifact.

## Lifecycle

1. **`/0-osp-onboarding`** scaffolds `session/raw/00_review_guidelines.md`, populates `session/session.json.qa_criteria`, pre-creates empty `session/raw/05_qa_<slug>.md` files (with the criterion definition as a header), and ensures `session/input/` contains a paper.
...
3. Each step updates the matching `phases.<name>` block in `session/session.json` on completion.
