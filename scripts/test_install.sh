...
# init_scripts.sh — pip install is exercised separately.
#
...
stub_init_scripts() {
  # Replace init_scripts.sh with a fast no-network stub for the duration of the test.
  local target="$1/scripts/init_scripts.sh"
  cat > "$target" << 'STUB'
#!/usr/bin/env bash
...
run_install_smoke() {
  ...
  # Copy the entire repo to a writable temp location so we can stub init_scripts.sh
  cp -r "$REPO_ROOT/." "$repo_copy/"
  stub_init_scripts "$repo_copy"
