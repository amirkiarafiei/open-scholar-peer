...
### D34 · Pi has no MCP, so the search layer grew a second surface — 2026-09-21

**Considered:** drop Pi as unsupportable / ship the prompts and accept that the literature phase retrieves nothing / depend on the third-party `pi-mcp-adapter` and write a `.mcp.json` for it / expose the same tools as a command-line program
**Chose:** the program, `mcp-server/osp_cli.py`.
**The measurement.** Pi's built-in tool set is `read`, `bash`, `powershell`, `edit`, `write`, `grep`, `find`, `ls` — stated identically in two vendor documents. There is no web search and no URL fetch, and MCP is an explicit non-feature: *"No MCP. Build CLI tools with READMEs (see Skills)."* So Pi cannot reach a paper database by any route we get for free, and "support Pi" would otherwise have meant shipping a review protocol with nothing to review against.
**Why not the third-party adapter.** It works and it reads a Claude-shaped `.mcp.json`, but it is one person's package outside the vendor's control, and OSP would be writing config for software the user has not installed. A dependency we cannot test is the failure this project already paid for once (D31).
**Why this is cheap.** `mcp-server/providers/` was already a plain Python package and `osp_mcp.py` only a thin FastMCP wrapper, so the bridge adds no second implementation: it reads the registered tool list back off the server at run time and calls the same function objects. Same 22 tools, same `OSP_SOURCES` gating, same error envelope. Verified live against arXiv, and against all five malformed-input paths.
**What it protects.** The exit code separates "the search ran and matched nothing" (0) from "the provider refused" (1) — the distinction M11 spent a milestone establishing, which a new calling convention could easily have thrown away. A database switched off by `OSP_SOURCES` says so in words rather than returning an empty list.
**Recorded because** a future reader will find a CLI and an MCP server doing the same job and assume one is legacy. Neither is: 20 tools use the server, one uses the program, and the program exists because a vendor said no on purpose.

### D35 · On three tools a slash command is a skill, so that is how we ship it — 2026-09-21

**Considered:** skip the tools that have no command files / write command files anyway and hope / emit the 8 commands as skill directories
**Chose:** emit them as skills, behind a `commands_as_skills` flag in the capability matrix.
**The measurement.** Hermes has no file-based reusable prompts at all — *"Put longer reusable prompts in a skill"* — and every installed skill automatically becomes a slash command. OpenClaw is the same: *"User-invocable skills are exposed as slash commands."* Cline still resolves workflows in code but has dropped them from its documentation and moved slash-command duty to skills.
**Why it is safe.** All three require exactly `name` and `description`, and Cline additionally requires `name` to equal the directory name — so the directory name alone decides the command, and no vendor-specific frontmatter had to be invented. Hermes slugification keeps a leading digit, so `0-osp-onboarding` survives as `/0-osp-onboarding`; checked in its source, not assumed.
**The bug this nearly caused.** `clean_adapter.sh` deleted only `osp-*/` skill directories. On these three tools the commands live beside the personas as `0-osp-*/` and `open-scholar-peer/`, so a re-install after a rename would have left orphans the sync could never remove. Found while writing the cleanup, not by a test.

### D36 · Where a subagent cannot be a skill, the persona ships twice — 2026-09-21

**Considered:** let the three tools fall back to self-reflection / inline the persona into the delegation message / generate each persona a second time as an agent definition
**Chose:** generate the agent definition, via an `agent_dir` field.
**The measurement.** Oh My Pi, Grok Build and Kilo Code all delegate to a child with a genuinely separate context window, and all three resolve the target from an agent-definition file — `.omp/agents/*.md`, `.grok/agents/*.md`, `.kilo/agents/*.md`. None can dispatch a skill that way; omp's resolver "searches only AgentDefinitions", and Grok's `subagent_type` resolves only to built-ins or agent files.
**Why not accept self-reflection.** It is a documented weaker substitute and these three are fully capable of the real thing. Taking the weaker mode because of a file-naming difference would have been a defect dressed as a limitation — and invisible, since the artifact would simply record `Mode: self-reflection`.
**Why it costs almost nothing.** Our skills already carry `name` and `description` frontmatter, which is exactly what an agent definition needs, so the file is the same content at a different path. Only Kilo Code wanted an extra key (`mode: subagent`), which is the one line of per-tool special-casing.
**Left as prefer-subagent, not subagent:** Hermes, OpenClaw and Antigravity document delegation but not that a persona is reachable through it, so they keep the try-then-degrade banner. That is the same call as D17 and the same open question, O11.

### D37 · The always-on rules stopped naming tools — 2026-09-21

**Considered:** extend each hardcoded list from 14 names to 21 / replace the lists with the principle
**Chose:** the principle.
**What was there.** `osp-rules.md` named which tools support subagents and which file-reference syntax each uses; `5-osp-qa.md` and `osp-query-agent/SKILL.md` each named "Mistral Vibe and OpenHands" as the self-reflection tools. All four statements were already wrong the moment Pi and Cline arrived, and one of them was wrong in the worst way — it told a Pi user that subagents were available.
**The rule these break** is the one M14 established for search tools: an inventory in a prompt is a promise the prompt cannot keep, and it rots silently because nothing tests prose. This was the same defect in a different file, and it survived M14 because M14 was scoped to source selection.
**What replaced it.** The Q&A mode now points at the banner `sync_adapters.py` already injects per tool, which is generated from the measured capability and therefore cannot disagree with the code. File references say "use whatever syntax your own tool provides", with two examples rather than a table.
**Measured after:** zero tool *inventories* remain in `extensions/_shared/`. Three named mentions
survive, all inside a conditional clause, which is precisely what M14 permits: *"Claude Code reads PDFs,
for one"* as an example of a host tool with native PDF support, `#file:` *"on Copilot CLI"* as an example
of a reference syntax, and *"TOML for Gemini CLI"* describing a transform. None of them promises a tool
is present. Adding the twenty-second tool will not require editing a prompt file.

**Corrected after review.** The first version of this entry claimed *zero tool names*, which was false —
the grep behind it searched six names and missed `Antigravity` and `MANIFEST.md` entirely. The reviewer
caught it, and with it two real stale lists: `5-osp-qa.md` named Antigravity alone as the
prefer-subagent tool when there are now three, and `MANIFEST.md` carried two per-tool tables covering 6
of 21, one cell of which had said "TBD" since Phase 5. Both are gone; the manifest now points at the
capability matrix rather than restating it. **A claim of "zero" is worth exactly as much as the command
behind it** — rule 7 applies to a grep, not only to a count.

### D38 · An exit code is not evidence that a config was written — read it back — 2026-09-21

**Considered:** trust `<tool> mcp add`'s exit status, as the Codex installer already does / verify by reading the config back after every CLI-based wiring
**Chose:** read it back, wherever the vendor's own CLI does the writing.
**The measurement, from a reviewer who had the tool installed.** `hermes mcp add` is interactive even for a stdio server: it asks before overwriting, probes the server, then asks which tools to enable. It hung for **3m18s** in a sandbox install. Worse, every exit path in its dispatcher returns `None`, which the shell reads as **0** — including the cancelled path. So an install that wired nothing printed `✅ MCP servers configured`. `openclaw mcp add` fails differently: it refuses a name that already exists and has no `--force`, so a re-install, a moved project or a rebuilt venv could never update a stale path.
**Why this is a rule and not two bug fixes.** OSP's whole MCP position since D31 is *we configure it ourselves rather than asking the user*. Delegating to a vendor CLI is still us configuring it — so the same standard applies, and the standard cannot be "the command exited 0". Both installers now remove-then-add, feed answers on stdin, bound the call with `timeout`, and decide the success message from `mcp list`.
**Cost accepted:** two extra subprocess calls per install, and a read-back that can itself fail on a tool that has no `list`. Cheap against a green tick on an install with no search tools.
**Not applied to** `codex mcp add`, which is documented non-interactive and has behaved for two milestones. If it ever grows a prompt, this is the pattern.
**The blind spot that let both through:** `test_install.sh` cannot exercise a vendor CLI, because the CLI is not installed in CI, so it asserts only the files OSP writes directly. That limitation is now stated at the top of the file rather than left to be rediscovered.

### D39 · CLI mode gets a `batch` call, because the process boundary is the bug — 2026-09-21

**Considered:** give the literature skill a CLI-mode exception telling it to dispatch sequentially / add a `batch` subcommand so a round runs in one process
**Chose:** `batch`. Resolves **O24**.
**The conflict.** `osp-literature-review-agent/SKILL.md:69` tells the agent to fire every provider in one dispatch. M18's C5 serialises arXiv and restores its three-second gap, so a parallel dispatch in CLI mode would take `ArxivBusy` after `_LOCK_WAIT` and record an unavailable source — a **false gap in the corpus**, which is the exact failure the error envelope exists to prevent.
**Why the sequential-skill option loses, and it is not close.** It does not avoid the cross-process work: sequential *separate* processes still start with fresh state every time, so the flock, the timestamp file and the on-disk cache are all still needed. You give up parallelism and still pay for the coordination. Measured cost: a three-round phase is ~18 calls at 0.44 s of start-up each, plus a 3 s enforced gap between arXiv calls — **over a minute of pure overhead per phase**, against ~0 for batch.
**Why batch wins.** The difference between MCP mode and CLI mode is one thing: **the process boundary**. One MCP server is one process, so the lock, the gap, the eight-entry text cache and the in-flight de-duplication all work. Batch removes the boundary for the common path, and every one of those mechanisms starts working again with no new machinery. It also needs no change to a battle-tested, tool-agnostic prompt — the skill keeps saying "dispatch them together", which stays true on both surfaces.
**The owner's worry, answered.** He flagged timeouts and rate limits. On rate limits batch is the *fix*, not the risk — not batching is what removes the throttle. On timeouts the mitigation is in the design: a per-item deadline rather than one for the batch, one envelope per item so a failure is attributed to its own call, never abort the batch on a single failure, and a bounded batch size.
**What it lets us drop.** C5 shrinks to a guard for the non-batch path: `fcntl.flock` plus a persisted timestamp. The on-disk parsed-text cache moves to O26, because batch already covers the common case and paging through one paper is the only remaining exposure.

### D40 · The canonical files keep a path that resolves nowhere, on purpose — 2026-09-21

**Considered:** write the real path into `_shared/` / drop the pointer and inline the template into every prompt that needs it / keep the short form canonical and substitute the real path at sync time
**Chose:** substitute at sync time. Closes **O23**.
**The defect.** Twenty-nine references across the canonical commands, skills and rules say `` `defaults/phase_block_template.md` ``. Nothing is ever installed at `<project>/defaults/` — every installer copies the adapter into `.claude/`, `.codex/`, `.agents/` and so on. **Measured: dead on all 21 tools**, not the 13 an outside reviewer first counted, because the path fails from the project root regardless of where that tool keeps its rules. The file it points at is the *only* definition of the phase block, so an agent that followed the pointer found nothing and had to invent the format.
**Why not write the real path into `_shared/`.** There is no single real path. It is `.claude/defaults/` for one tool and `.agents/defaults/` for another, and OpenClaw installs into a directory that is not even its own adapter's name. A canonical file that names one tool is the thing M14 and D37 spent effort removing.
**Why not inline the template.** It is ~96 lines and would land in sixteen prompt files — the duplication M16 existed to delete.
**So the short form stays canonical and is wrong there deliberately**, and the sync rewrites it per tool. A reader of `_shared/` should know that: the path is a *name*, resolved at generation time, not a location.
**The bug inside the fix, worth more than the fix.** The first version derived the prefix from `tool.root.name`. `sync_adapters.py --check` clones every tool with `dataclasses.replace(root=tmpdir/name)`, so the check generated `claude/defaults/…` while the real run generated `.claude/defaults/…` — 20 of 21 tools drifted, and only OpenClaw passed because it alone had an explicit value. **A field derived from another field is right in one code path and wrong in the next.** `install_dir` is now stated explicitly on all 21 and raises rather than guessing. Caught by the drift check, not by me.
**Guarded:** `test_parity.py` fails on any bare `defaults/…` reference in generated output, fault-tested.

---

### D41 · The CLI derives its own schema, and neither surface owns one — 2026-09-22

**Considered:** keep importing `osp_mcp` just for the schemas / hand-write 22 JSON Schemas for the CLI / derive from `inspect.signature` in the CLI
**Chose:** derive from the signature.
**Why the first loses.** It is what M18 exists to remove. The CLI read its *entire* surface — tool list, descriptions and schemas — off FastMCP's pydantic `Tool` objects, so it could not stop importing `mcp` until it could describe a tool itself. C2 was impossible before C3; the recorded order C0→C1→C2→C3 could not be executed and became C0→C1→C3→C2.
**Why the second loses, and why the wording matters.** Twenty-two hand-written schemas would be the "two artifacts a human keeps in step" drift this project has already paid for twice. It is worth stating positively, because a future reader seeing two derivers will reach for exactly that: **there is one source — the decorated function's signature — and two readers of it.** A human edits one function and both surfaces follow.
**The measurement that made it safe.** The annotation space is closed and tiny: exactly seven forms across all 22 tools (`str` ×23, `int` ×17, `str|None` ×9, `int|None` ×5, `bool` ×4, `list[str]|None` ×4, `list[str]` ×1). The deriver is ~40 lines and its output is **byte-identical** to pydantic's for all 22 — which took two non-obvious rules: pydantic sorts keys alphabetically at both levels, and each field carries `name.replace("_"," ").title()`.
**What the test pins, and why the obvious test would have been worthless.** Argument names and required sets already matched 22/22, so comparing only those is a test that cannot fail. `test_schema_parity.py` compares types and defaults field by field, *then* the whole document byte for byte, *then* a frozen census of the seven forms — so a 23rd tool with an eighth form fails with a one-line diff before anyone runs the CLI.
**Verified closed:** `FastMCP.tool()`, `add_tool()` and `Tool.from_function()` accept no schema argument, so handing pydantic our schema is not an option.

### D42 · Providers load lazily, because the fallback must outlive its dependencies — 2026-09-22

**Considered:** import all six providers eagerly in `core` / a PEP 562 module `__getattr__` proxy / import inside each of the 22 tool bodies / `importlib.util.LazyLoader`
**Chose:** `LazyLoader`.
**The PEP 562 option is not a trade-off, it is broken.** Tested: a module `__getattr__` is **not consulted for a `LOAD_GLOBAL` inside a function defined in that module**. The tool bodies say `arxiv_provider.search`, so every one would have raised `NameError`. It would have worked only by rewriting all 22 bodies — destroying the byte-identity that made the extraction verifiable.
**Why eager loses, measured.** Criterion 2 wanted `call` ≥70% below a 445 ms baseline. Eager: ~169 ms, **62%** — fails. Lazy: 104 ms on the heaviest provider, **76.6%** — passes. For `list`, `import asyncio` alone costs 37 ms against a 44.5 ms budget, so `asyncio` also had to move inside `_run` and `cmd_call`; `list` then measures 38 ms, **91.5%**.
**But speed is not the reason, and the commit says so.** 0.3 s of import sits against 2–6 s of provider latency. The reason is criterion 1: before this, blocking any one of `mcp`, `arxiv`, `requests`, `bs4` or `semanticscholar` made `import osp_cli` raise — **exit 1 with zero bytes of stdout**, while the contract says exit 1 means "read the `reason`". Zero bytes is the worst possible answer, indistinguishable from a tool with nothing to say. After: `list` survives all six blocked in turn, and a `call` against the broken one degrades to an envelope.
**The cost, accepted and mitigated.** A genuine packaging breakage now presents as several `unavailable` envelopes rather than one loud failure, so an agent could read "five databases are down" instead of "this install is broken". `_err` therefore spells it out for an `ImportError`: *"a missing dependency in the install, not a problem with the database."* And the long-lived MCP server calls `warm_providers()` at start-up, so its behaviour is unchanged and no two concurrent calls race to be first to touch a cold module.

### D43 · The error envelope matches exception names, not classes — 2026-09-22

**Considered:** keep `isinstance` and accept that `core` imports all six providers / a reason table owned by each provider / match class names along `type(exc).__mro__`
**Chose:** the MRO name walk.
**The problem `isinstance` creates.** It needs the exception classes, so building an error report imports the six providers — meaning **a broken dependency breaks the report of itself**, and the report is the only thing standing between "the provider failed" and "there are no papers". It also undoes D42 on the error path.
**Why a per-provider table loses.** Reading it requires importing the provider, which re-creates the problem exactly, and it scatters an agent-facing vocabulary across six files.
**Why this is not a new idea here.** `_err` already matched the `semanticscholar` package's exceptions by name, precisely so a missing optional dependency could not break error reporting. This generalises a pattern the file had already chosen.
**Verified, not assumed.** Compared against the previous implementation over **27 exception types** — every class the six providers export, both `semanticscholar` types, and eight builtins: **zero divergences in reason and in message**. The equivalence is structural rather than lucky: every subclass relationship among them (`Blocked ⊂ Unavailable`, `ArxivNotFound ⊂ ArxivFullTextError`, `ZenodoRateLimited ⊂ ZenodoError`, …) puts the more specific name first in the MRO, which is the precedence the hand-ordered `isinstance` chain encoded.
**The risk, and the guard.** A provider renaming an exception would silently downgrade its reason to `failed` — quiet, which is the failure mode this layer exists to remove. Three checks now fail loudly instead: every name in the table resolves to a real class, no two providers export the same name, and every provider exception has a reason. Fault-tested both ways. It found one thing immediately: `europe_pmc._EntityDeclared`, which is raised and caught inside one function and correctly excluded by its leading underscore.

### D44 · Gating happens when asked, not at import — 2026-09-22

**Considered:** keep `_ENABLED = _enabled_sources()` as a module constant and have the test clear `sys.modules` / compute the enabled set on each call
**Chose:** compute on call.
**A measurement changed our minds.** The plan expected to *work around* a hazard: `test_providers_unit.py` re-executes the front end three times with different `OSP_SOURCES` and asserts 22/17/3 tools, and after the split `import core` would hit `sys.modules` and return a cached module with stale gating. The fix considered was `del sys.modules["core"]` in the test. Making gating a function removed the hazard instead — **the test needed no edit at all and passed first time.**
**Two things it fixed that were not the goal.** A constant is fixed before any front end has configured logging, so the "OSP_SOURCES names X, which is not a source I know" warning was formatted by Python's handler of last resort; it now reaches the front end's own handler. And a test working around the design is a test that breaks again the moment a third caller appears.
**The principle worth keeping:** when a test needs to defeat module caching, the value it is fighting probably should not have been frozen at import.

### D45 · A truncated result is not a failed one — 2026-09-22

**Considered:** let the host truncate and hope / return an error envelope when the result is too large / return the records that fit plus a marker that says so
**Chose:** the marker, and exit **0**.
**The failure being prevented, in a reviewer's words.** *"The agent runs `search_arxiv max_results=50`, the host truncates 105 KB mid-JSON, it salvages twelve papers and writes Provenance saying nothing was missing — exit 0, no `reason`, and a review built on a quarter of the corpus."* Measured: that call writes **103,399 bytes**, about 24,500 tokens.
**Why not an error envelope.** A truncated search is not a failed one. The records that arrived are complete and real, and an agent treating the result as an error would throw away good papers — trading a silent partial corpus for a silently discarded one. So the exit code stays 0 and the truth sits in the data, where an agent that parses the result at all must see it.
**Why not the error `reason` vocabulary either.** The marker uses its own keys (`osp_truncated`, `osp_returned`, `osp_total`) rather than `error`/`reason`, so it cannot be mistaken for a failure by code that branches on the envelope shape.
**Shape is preserved:** a list stays a list and a record stays a record, so nothing downstream has to learn a second format. Measured after: 103,399 → 22,108 bytes, valid JSON, "11 of 50" declared.

### D46 · The CLI exits hard, and deliberately has no watchdog — 2026-09-22

**Considered:** return normally from `asyncio.run` / a watchdog thread that kills the process on a timer / write the result inside the event loop and `os._exit`
**Chose:** emit inside the loop, then `os._exit` after an explicit stdout flush.
**The measurement.** `asyncio.wait_for` cancels the *wait*, not the worker thread, and `asyncio.run()` **joins the executor on the way out**. A 1 s deadline against a 6 s call raised at 1.00 s and the process did not return until **6.01 s** — so returning normally holds the answer hostage to the thing that already timed out. After: an 8 s stuck worker with a 1 s deadline exits at **1.08 s** carrying `reason: timeout`.
...
### D52 · Antigravity CLI's commands are skills; the IDE's are not, and stay put — 2026-09-23

**Considered:** keep shipping `.agents/workflows/*.md` and treat the report as a vendor bug / emit the eight commands as skills, as three other tools already do / change both Antigravity entries together
**Chose:** skills for the **CLI only**. The IDE is untouched.
**The symptom.** The owner: *"antigravity cli does not see our prompts or slash commands. it only sees agents but the open-scholar-peer and numbered commands are not visible."* Persona skills visible, the eight commands invisible, from the same `.agents/` tree — and that asymmetry was the whole clue, because it meant one of the two paths we write was right and the other was dead.
**The measurement.** On `agy` 1.2.9: our tree as shipped gives a `/skills` catalog of **8** — the personas only — and typing a command name falls through to raw text, with the agent trying to run a *shell command* by that name. The same eight emitted as skill directories give **16**, and `/open-scholar-peer` activates. Verified again here on the real generated adapter, not a fixture.
**Why it broke.** Workflows are deprecated. The binary ships a `migrate-workflows` skill saying skills provide everything workflows did *"plus first-class slash command support"*.
**Nothing new had to be built.** `commands_as_skills` has existed since D35 for Hermes, Cline and OpenClaw. Antigravity CLI is the fourth tool where a skill *is* the slash command. One field.
**The cleanup that would otherwise have been missed,** and it is the same trap D35 caught: an existing install has our eight files in `.agents/workflows/`, which the tool has stopped reading. Nothing would ever remove them, so `clean_adapter.sh` gained a `legacy_cmd_subdir` sweep. **A layout we stop writing still has to be unwritten.**

**The vendor and the binary disagree, and we sided with the binary.** The migration page says workflows *"are deprecated and will be retired on November 1, 2026"*, and until then remain functional — its own table lists both formats as supporting slash commands. They are already gone in 1.2.9, about six weeks early. Independent confirmation: the prompt block that tells the model which workflows exist is **present in the IDE's language server and absent from `agy`** — the CLI did not disable the feature behind a flag, it removed the machinery. A reproducible vendor bug, worth filing, and nothing in our fix waits on it.

**Why the IDE is deliberately left on workflows.** It is a different product — `/usr/bin/antigravity` 1.107.0, config in `~/.gemini/antigravity/`, installing to `.agent/` — and it still carries the workflow machinery that `agy` removed. It also has no headless mode, so it cannot be tested the way the CLI was: the agent panel is GUI-only. Converting it would trade a mechanism with evidence behind it for one with none, and **whether the IDE turns a skill into a slash command at all is unknown** — the "[slash command] (path)" listing block that makes that work is workflow-specific. Left alone, and recorded as **O27** with the November date.
...
|---|---|---|---|
| **O27** | **The Antigravity IDE's slash commands retire on 2026-11-01, and we cannot test the replacement.** The vendor dates the retirement of workflows to that day; `agy` already removed them six weeks early (D52), so the IDE is likely to follow. We still ship its eight commands as `.agent/workflows/*.md`, which is correct today — the prompt block that indexes them is present in its language server and absent from the CLI's. Two things block a pre-emptive fix: the IDE has no headless mode, so nothing can be verified without a person driving a GUI; and **whether the IDE exposes a skill as `/<name>` at all is unknown** — the listing block that does it is workflow-specific, so converting could remove the commands rather than move them. Options: drive the GUI by hand before November; ship both layouts for a period and accept the duplication; or wait for the vendor to document the IDE's skill-to-command path. Until then this is a dated time bomb, not a bug. | 2026-09-23 | open |
| **O26** | **CLI paging through one paper still re-downloads it.** D39 moved the common case to `batch`, which runs a round in one process and so keeps the eight-entry parsed-text cache. Paging remains exposed: `read_arxiv_paper` windows at 50,000 chars, so a 300k-char paper is six calls an agent may issue one at a time, and each fresh process re-downloads and re-parses the tarball — the regression D25/O17 removed. Options: an on-disk cache under `.open-scholar-peer/mcp/cache/` with the same cap and a TTL; a larger default window on the CLI surface; or telling the agent to batch its windows. Until one is chosen, `KNOWN_LIMITATIONS.md` must say it plainly. | 2026-09-21 | **open** — M18 shipped the rest of C5 (`fcntl.flock` carrying arXiv's timestamp across processes, verified: 3.01 s apart and never concurrent) and `batch`, which covers the common case. Paging is the one remaining exposure and is now stated plainly in `KNOWN_LIMITATIONS.md` §9. |
| **O25** | **The CLI fallback may be unavailable exactly where it is needed.** A reviewer reports that a sandboxed host can block outbound HTTP from `bash` while still permitting the MCP server's own connections. If that holds, CLI mode does not work there at all, and M18's whole premise — that the CLI covers a tool whose MCP is missing — fails on those hosts. **Probed 2026-09-21. The concern is real, and it misses where it would have mattered.** Measured on Claude Code: bash reaches the network (HTTP 200 in 479 ms; sockets to arXiv and Semantic Scholar in 82 and 111 ms). Documented for the rest. **Shell network blocked by default on three:** Codex CLI — *"By default, the agent runs with network access turned off"*, `[sandbox_workspace_write] network_access = false`, confirmed in code by the test `read_only_on_request_blocks_network`; Antigravity **IDE** on macOS/Linux — sandbox on by default with a **per-domain** outbound allowlist, so all five providers would need approving one by one; and Kiro **Web** at its baseline tier. **Allowed on the rest:** OpenHands, Antigravity CLI (`enableTerminalSandbox` defaults false), Kiro local IDE and CLI, Cline, Kilo Code, Junie, and **Pi, which has no sandbox at all** — so the fallback works exactly where it is the *only* path. All three blocked tools have working MCP, so the fallback is unavailable precisely where it is not needed — but it also means **those three have no fallback if their MCP breaks**, which `KNOWN_LIMITATIONS.md` must say. **The sub-question that mattered most is answered: MCP subprocesses are not sandboxed on Codex**, so OSP's normal path works there and there is no shipping bug. Code-level evidence: `mcp_servers` stdio entries go `make_rmcp_client` → `LocalStdioServerLauncher`, spawned with a plain `tokio::process::Command` and no Seatbelt or Landlock wrapper, while shell calls go through `codex_sandboxing::spawn_process`. The sandbox attaches at the exec path, not at MCP spawn. **Caveat kept deliberately: no vendor document states this exemption**, so it is read from code and must be smoke-tested before anyone relies on it. For **Antigravity IDE** the same question is `NOT DOCUMENTED` either way — test, do not assume. **The probe, verified here 2026-09-21 in all three cases** — reachable 572 ms, blocked 5,091 ms, and a proxy-style block where TCP connects but HTTPS is refused: `python3 -c "import urllib.request as u;u.urlopen('https://api.openalex.org/works?per-page=1',timeout=5).read(1);print('NET_OK')" 2>/dev/null || echo NET_BLOCKED`. Three design points behind it, each a real failure otherwise: the timeout is **internal**, because coreutils `timeout` is absent on macOS and its absence would read as blocked; Python rather than `curl`, because a missing `curl` also mimics a block; and a real HTTPS fetch rather than a TCP connect, because a proxy accepts the connect and then refuses. Probe a host you will actually call — Antigravity allowlists per domain. If true, the honest behaviour is one clear message, never a silent empty corpus. | 2026-09-21 | open |
| **O24** | **Serial CLI dispatch contradicts the protocol.** `osp-literature-review-agent/SKILL.md:69` tells the agent to fire every provider in one batch. M18's C5 serialises arXiv behind a cross-process lock and restores the 3 s gap, so a parallel dispatch would hit `ArxivBusy` after `_LOCK_WAIT` and be recorded as an unavailable source — a false gap in the corpus, which is the one thing the error envelope exists to prevent. Either the skill gains a CLI-mode exception, or `batch` stops being deferred. | 2026-09-21 | **closed 2026-09-21 (D39)** — `batch`. The sequential-skill option does not avoid the cross-process work and costs over a minute per phase. |
| **O23** | **A bare `defaults/…` pointer is already dead on 13 of 21 tools.** Measured 2026-09-21: those tools merge their rules into the project-root `AGENTS.md`, while `defaults/` is written under `.<tool>/`, so a relative pointer from the rules resolves to nothing. This already ships for `phase_block_template.md`. Not introduced by M18, but M18's C8 would repeat it. Fix the existing case and decide the convention: an absolute-from-project-root path, a per-tool path written at sync time, or inline the text. | 2026-09-21 | **closed 2026-09-22 (D40)** — substituted at sync time via `install_dir`; canonical keeps the short form. Measured dead on all 21 tools, not 13. Guarded in `test_parity.py`. **The 2026-09-21 closure was premature and the report of it was wrong.** The guard and the grep behind it matched only the backtick short form `` `defaults/x.md` ``. Two canonical lines spelled the same defect `extensions/_shared/defaults/…` — a repo path no user has — and reached **45 generated files** unseen. Fixed 2026-09-22. Same lesson as D37: a claim of zero is worth exactly as much as the command behind it, and one grep does not cover two spellings. |
