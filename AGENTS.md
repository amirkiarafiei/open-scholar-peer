...
> *(This file is for agents working on OSP itself — NOT the file end users get in their project. The user-facing `AGENTS.md` is generated by `install_copilot.sh` from `extensions/_shared/rules/osp-rules.md`.)*
...
# Smoke-test all 5 installers in temp dirs (must pass before tagging release):
...
**Never edit `extensions/.{claude,cursor,gemini,agent,agents,github}/` directly.** They are generated. Edit `extensions/_shared/` and re-run sync.
...
extensions/.{claude,cursor,gemini,agent,agents,github}/   ← Generated. Don't edit.
